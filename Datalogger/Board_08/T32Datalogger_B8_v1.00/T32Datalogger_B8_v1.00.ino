...
#define KEY_PIN A7    // key status pin
...
float TempA = 0.0f;                       // ambient temp from alt sensor
float BarPressure = 0.0f;                // barametric pressure from alt sensor
...
uint8_t hrD0ID, hrD1ID, hrD2ID, hrCarID, hrMotorID, hrTempA, hrBarPres;
...
void setup() {
  ...
  Display.setCursor(STATUS_TYPE, 120); Display.print(F("Alt / GPS"));
  ...
  hrTempA = SSD.addHeaderField("Amb Temp",  &TempA);
  hrBarPres = SSD.addHeaderField("Bar Press",  &BarPressure);
...
void ComputeData() {
  ...
  if (GPSTolerance > 0) {
    ...
    sensor.read();
    Altitude = sensor.altitude() * METERS_TO_FEET;
...
uint16_t RestoreRaceData() {
  ...
  Altitude = SSD.getField(Altitude, frAlt) * METERS_TO_FEET;
...
#ifdef DO_DEBUG
  Serial.print("1725 LastRecord "); Serial.println(SSD.getLastRecord());
  Serial.print("RecordType "); Serial.println(RecordType);
  Serial.print("RecordSETID "); Serial.println(RecordSETID);
  ...
  Serial.print("Altitude "); Serial.println(Altitude) * METERS_TO_FEET;
...
void CreateMenus() {
  ...
  CalMenuOption3 = CalMenu.addNI("Amp slope (26.4/20.0)", mVPerAmp, 15, 30.0, 0.1, 1);
  CalMenuOption4 = CalMenu.addNI("Amp offset (.396/.500)", VMid, .3, 0.6, 0.001, 3);
  ...
  CalMenuOption10 = CalMenu.addNI("Pickups",  Pickups, 1, 50, 1);
...
void ProcessTranMenu() {
  ...
  while (MenuOption > 0) {
    ...
    if ((millis() - caltime) > 1000) {
      ...
      Display.print(Altitude, 0); Display.print(" / "); Display.print(GPSSatellites);
...
void DumpAllDataToScreen() {
  ...
  for (i = 1; i <= LastRecord; i++) {
    ...
    if (rt == RT_HEADER) {
      ...
      Serial.print(SSD.getField(Altitude, frAlt ) * METERS_TO_FEET); Serial.print(", ");
...
void DownloadRaceData() {
  ...
  for (i = 1; i <= LastRecord; i++) {
    ...
    if (rt == RT_HEADER) {
      ...
      SDDataFile.write(DATA_DELIMITER); SDDataFile.print(F("Altitude"));
    ...
    if (rt == RT_DATA) {
      ...
      SDDataFile.write(DATA_DELIMITER); SDDataFile.print( SSD.getField(Altitude, frAlt  ) * METERS_TO_FEET, 1);
...
void DownloadGPSData() {
  ...
  for (i = 1; i <= LastRecord; i++) {
    ...
    if (rt == RT_DATA) {
      ...
      SDDataFile.write(44); SDDataFile.print(SSD.getField(Altitude, frAlt  ) * METERS_TO_FEET, 1);
      // convert to MPH
...
void TestSensors() {
  ...
  if (!sensor.init()) {
    Display.setTextColor(C_RED);
    Display.print(F("ALT FAIL"));
  }
  else {
    sensor.read();
    Display.setTextColor(C_GREEN);
    Display.print(sensor.altitude() * METERS_TO_FEET, 0);
  }
  Display.print(F(" / "));
