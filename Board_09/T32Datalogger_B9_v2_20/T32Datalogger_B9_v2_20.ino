...
void setup() {
  ...
  Serial.print("Record number for RecordSet: ");
  Serial.print(TR);
  Serial.print(" is ");
  Serial.println(SSD.getFirstRecord(hrID, TR));
...
*/
  ...
  // in minutes
  Duration = ((hour() * 60) + minute()) - ((RaceHour * 60) + RaceMinute);
...
void ComputeData() {
  ...
  // restore = true use in Restore Race Data
  RestoreType = false;
  ...
  /*
  Serial.print("RACE_TIME_SECONDS ");
  Serial.println(RACE_TIME_SECONDS);
...
void CheckIfPitStop() {
  ...
  if ((DriverTime[Driver] > 900000l) && (Amps > DriverChangeAmps) && (CarSpeed < 15.0f)) {
    ChangeDriver();
  }
}
...
uint16_t RestoreRaceData() {
  ...
  if (DriverTime[0] > 0) {
    DriverTimer = DriverTime[0];
  }
  if (DriverTime[1] > 0) {
    DriverTimer = DriverTime[1];
  }
  if (DriverTime[2] > 0) {
    DriverTimer = DriverTime[2];
  }
  ...
  CarRaceTimer = (Duration * 60000l) + TotalDownTime;
...
void CreateUserInterface() {
  ...
  SensorMenuOption14 = SensorMenu.addNI("Adjust altitude per lap", ResetAltitude, 0, sizeof(YesNoText) / sizeof(YesNoText[0]), 1, 0, YesNoText);
...
void ProcessSensorMenu() {
  ...
  ResetAltitude = SensorMenu.value[SensorMenuOption14];
