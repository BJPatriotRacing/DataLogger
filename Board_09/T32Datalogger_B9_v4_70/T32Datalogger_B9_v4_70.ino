/*
  ...
  Code for Teensy 3.2 / 4.0 (with change to RX definition)
  ---------------------------------------------------------
...
#define CODE_VERSION "B9_v4.70-CYBORG"
...
uint8_t MainMenuOption4 = 0, MainMenuOption5 = 0, MainMenuOption6 = 0, MainMenuOption7 = 0, MainMenuOption8 = 0;
...
uint8_t frHumidity = 0, frAmbTemp = 0, frCyborgInSignal, frCyborgOutSignal;
...
void setup() {
  ...
  // call the interrupt to set banner state and color
  HANDLE_KEY_CHANGE();
...
void loop() {
  ...
  if (EnableCyborg) {
    ...
    if (CyborgTimer > CyborgUpdateTime) {
      ...
      if (CyborgAmps > CyborgMaxCurrent) {
        // CyborgOutputPWM = (CyborgMaxCurrent / CyborgAmps) * BIT_CONVERSION;
        if (CyborgOutputPWM > 10) {
          CyborgOutputPWM = CyborgOutputPWM - 10;
        }
      } else {
        if (CyborgInputBits > 4080) {
          CyborgOutputPWM = CyborgOutputPWM + 10;
        } else {
      ...
      CyborgInSignal = (CyborgInputBits * 100) / BIT_CONVERSION;
      CyborgOutSignal = (CyborgOutputPWM * 100) / BIT_CONVERSION;

      // kris
      /*
      Serial.print(CyborgCounter);
      Serial.print(", ");
      Serial.print(CyborgOutputBits);
      Serial.print(", ");
      Serial.print(CyborgInputVolts);
      Serial.print(", ");
      */

      Serial.print(CyborgAmps);
      Serial.print(",");
      Serial.println(CyborgOutputPWM);
...
*/
  ...
  if (DisplayUpdateTimer >= UPDATE_LIMIT) {
    ...
    if ((Amps >= TriggerAmps) && (AllowDriverChange) && (digitalRead(KEY_PIN) == HIGH)) {
...
void GForceView() {
  ...
  Display.fillRect(10, 139, 300, 2, C_LTGREY);   // horizontal
  Display.fillRect(160, 40, 2, 200, C_LTGREY);   // vertical
...
void CreateUserInterface() {
  ...
  CyborgMenuOption6 = CyborgMenu.addNI("Cyborg Update", CyborgUpdateTime, 50, 500, 50, 0);
  CyborgMenuOption1 = CyborgMenu.addNI("Max current Limit", CyborgMaxCurrent, 8.0, 50.0, 0.1, 1);

  CyborgMenuOption3 = CyborgMenu.addNI("Kp (Converge time)", Kp, 0, 500, 1, 0);
  CyborgMenuOption4 = CyborgMenu.addNI("Ki (Creep time)", Ki, 0, 500, 1, 0);
  CyborgMenuOption5 = CyborgMenu.addNI("Kd (Overshoot)", Kd, 0, 500, 1, 0);
...
void DownloadRaceData(uint32_t Count) {
  ...
  for (i = 1; i <= LastRecord; i++) {
    ...
    if (rt == RT_DATA) {
      ...
      // volts
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(SSD.getField(Volts, frVolts), 3);
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(SSD.getField(Amps, frAmps), 3);
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(SSD.getField(MotorTemp, frMotorTemp), 3);
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(SSD.getField(AuxTemp, frAuxTemp), 3);
      SDDataFile.write(DATA_DELIMITER);
      ...
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(SSD.getField(CarSpeed, frSpeed), 3);
      SDDataFile.write(DATA_DELIMITER);
      ...
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(SSD.getField(Altitude, frAlt), 2);
      SDDataFile.write(DATA_DELIMITER);
...
void BuildFieldList() {
  ...
  frGPSAlt = SSD.addField("Alt", &GPSAltitude);
  ...
  frCyborgInSignal = SSD.addField("Cyborg Input", &CyborgInSignal);
  frCyborgOutSignal = SSD.addField("Cyborg Output", &CyborgOutSignal);
  ...
  hrTirePressureID = SSD.addHeaderField("Tire Pressure", &TirePressure);
...
void InitializeSensors() {
  /*
  while (1) {
    CyborgOutputBits = analogRead(OUTPUTVOLT_PIN);
    Serial.println(CyborgOutputBits);
    delay(100);
  }
  */
  analogReadRes(12);
  ...
  // create interrupt for the key state routine
  attachInterrupt(digitalPinToInterrupt(KEY_PIN), HANDLE_KEY_CHANGE, CHANGE);
}

void HANDLE_KEY_CHANGE() {
...
void CalibrateAccererometer() {
  ...
  while (1) {
    ...
    Tries++;
    StatusBarWidth = (float)((Tries / 50.0) * 293.0);
