...
#define AX_PIN 14   // aus pin for EBYTE
...
uint8_t WirelessMenuOption5 = 0, WirelessMenuOption7 = 0;
...
void SendData() {
  ...
  // lap time (LT) data stored in ms, GPS distance (distance from start) in meters
  if (GPSDistance > 512) {
  ...
  while (MenuOption > 0) {
    ...
    if (digitalRead(LPin) == LOW) {
      ...
      if (val == LONG_PRESS) {
        ...
        if (MenuOption == GForceMenuOption8) {
