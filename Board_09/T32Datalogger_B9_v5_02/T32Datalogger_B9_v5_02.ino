...
#include <Arial_48BINO.h>             // 1000 point bold italics numbes only, a special font file that is numbers only. stored in PatriotRacing_Fonts, recreate use https://spooksoft.pl/download/, https://spooksoft.pl/en/download-3/
...
volatile uint8_t fnum = 0, digit10 = 0, digit1 = 0;
...
#define FONT_48BINO Arial_48BINO        // font for the large data
...
float CarSpeed = 0.0f, CarMaxSpeed = 0.0f, OldCarSpeed = 0.0f;  // computed speed
...
uint8_t RaceMenuOption11 = 0, RaceMenuOption12 = 0, RaceMenuOption13, RaceMenuOption14, RaceMenuOption15;
...
void PrintTheScreen() {
  ...
  digit10 = fnum / 10;
  digit1 = fnum % 10;
  //FileName[0] = digit10 + '0';
  //FileName[1] = digit1 + '0';

  sprintf(buf, "%d%d.bmp", digit10, digit1);
  ...
  delay(500);
  Serial.println(SaveBMP24(&Display, 10, buf));
...
void loop() {
  ...
  if (DisplayID == 7) {
    if ((GForceTimer > 200) && (DriverChangeScreen > 3000)) {
      GForceTimer = 0;
      if (RedrawDisplay) {
        Display.fillScreen(back_color);
        RedrawHeader = true;
...
void ComputeSpeed() {
  ...
  if ((CarSpeed > 50.0f) || (CarSpeed < 0.0f)) {
...
void CheckIfStarting() {
  ...
  if (RaceStatus == RACE_NOTSTARTED) {
    ...
    Serial.println("-----------------------adding new record");
    AddNewRecordset();
...
void AddNewRecordset() {
  ...
  if (SSD.addRecord()) {
    ...
    Serial.println("added new record, saving");
    SSD.saveHeader();
  }
...
void CreateUserInterface() {
  ...
  RaceMenuOption13 = RaceMenu.addNI("Max Lap Amps", MaxLapAmps, 15, 21, .1, 1);
...
*/
  ...
  EnergyG.setLineThickness(GraphVoltsID, 2);
  EnergyG.setLineThickness(GraphLapAmpsID, 2);
  EnergyG.setLineThickness(GraphAmpsID, 2);
  EnergyG.setLineThickness(GraphSpeedID, 2);
  EnergyG.setLineThickness(GraphMTempID, 2);
  EnergyG.setLineThickness(GraphCyborgInID, 2);
  EnergyG.setLineThickness(GraphCyborgOutID, 2);
...
void ProcessSSDMenu() {
  ...
  while (MenuOption > 0) {
    ...
    if (ButtonPressed == L_BUTTON) {
      ...
      if (SSDMenu.item == 0) {
        ShowJEDECScreen();
      } else if (SSDMenu.item <= SSDMenuOption2) {
        DrawInfoScreen();
      } else if (SSDMenu.item > SSDMenuOption2) {
    ...
    if (ButtonPressed == C_BUTTON) {
      ...
      if (MenuOption == SSDMenuOption1) {
        ...
        delay(1000);
        DrawInfoScreen();
      } else if (MenuOption == SSDMenuOption2) {
        ...
        Display.print(F("Erasing blocks..."));

        Display.drawRoundRect(13, 182, 293, 36, 2, C_GREY);
        // Display.fillRoundRect(13, 182, 4, 36, 2, C_GREEN);
        for (i = 0; i <= UsedBlocks; i++) {
          Display.fillRoundRect(13, 182, (296 * i) / UsedBlocks, 36, 2, C_GREEN);
          SSD.eraseBlock(i);
          delay(100);
        }
        Display.fillRect(10, 140, 320, 35, C_WHITE);
        Display.setCursor(10, 150);
        Display.print(F("Resetting race..."));
        SaveStartGPS(false);
        ResetRaceDate();
        ...
        Display.fillRect(10, 140, 320, 35, C_WHITE);
        Display.setCursor(10, 150);
        Display.print(F("Erase complete."));
        delay(1000);
        DrawInfoScreen();
      }

      else if (MenuOption == SSDMenuOption3) {  //all
        DownloadRaceData(2);
        DownloadGPSData(1);
        DownloadEEPROM();
        DrawDownloadInfoScreen();
      } else if (MenuOption == SSDMenuOption4) {
        DownloadRaceData(1);
        DrawDownloadInfoScreen();
      } else if (MenuOption == SSDMenuOption5) {
        DownloadGPSData(1);
        DrawDownloadInfoScreen();
      } else if (MenuOption == SSDMenuOption6) {
        DownloadEEPROM();
...
void DrawInfoScreen() {
  ...
  Display.setCursor(210, 155);
  Display.print(SSD.getRecordLength());
  Display.print(F(" / "));
  Display.print(SSD.getDatabaseRecordLength());

  Display.setCursor(210, 175);
  Display.print(RecordSETID);
...
void DownloadEEPROM() {
  ...
  SDDataFile.print(F("Driver 1"));
  SDDataFile.print(F("Driver 2"));
  SDDataFile.print(F("Driver 3"));
