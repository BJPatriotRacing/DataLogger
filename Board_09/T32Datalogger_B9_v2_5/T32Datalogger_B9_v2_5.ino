...
float iTemp = 0.0f, iPressure = 0.0f, Altitude = 0.0f, TempAltitude = 0.0f, StartAltitude = 0.0f, Humidity = 0.0f;
...
void ComputeData() {
  ...
  if (AltimiterStatus) {
    ...
    TempAltitude = BMEsensor.readAltitude(SEALEVELPRESSURE_HPA);
    ...
    TempAltitude = TempAltitude * METERS_TO_FEET;
    Altitude = TempAltitude + AltitudeOffset;
  ...
  if ((Amps >= TriggerAmps) && (RaceStatus == RACE_NOTSTARTED) && (KeySwitchedOn)) {
...
void CheckIfPitStop() {
  // Auto driver change
  // Current driver has driven for over 15 minutes (900 seconds or 900,000 ms), drivertimeer is in ms
  // however if we are on the last driver, don't change
  // driver can press both buttons to force a driver change if needed (rare)
  // if Key was just switched on, it must be coming out of a pit change
  if ((DriverTime[Driver] >= 900000l) && (Amps >= TriggerAmps) && (KeySwitchedOn)) {
    KeySwitchedOn = false;  // reset the key switch flag so we can detect the next pit change
    ChangeDriver();
  }
}
...
uint16_t RestoreRaceData() {
  ...
  StartRecord = SSD.getFirstRecord(hrID, RecordSETID);
...
void TimeView() {
  ...
  // reminder, distance is in meters, odd since everything else is in Imperial
  // but...GPStolerance is in meters
  if (GPSDistance < 50) {
...
void CheckIfLap() {
  ...
  if ((StartGPSFound) && (GPSDistance <= GPSTolerance) && (RaceStatus == RACE_INPROGRESS) && (GPSLapTimer >= (1000l * LapThreashold))) {
    ...
    if (ResetAltitude) {
      AltitudeOffset = StartAltitude - TempAltitude;
    }
...
void ISR_KEYSTATE() {
  ...
  if (digitalRead(KEY_PIN) == LOW) {
    KeySwitchedOn = false;         // key turned off, entering pit, we
    RestoreType = STATUS_PITSTOP;  // this is also set in loop()
  } else {
    KeySwitchedOn = true;     // key turned on, exiting pit (or statrup)
    RestoreType = STATUS_OK;  // this is also set in loop()
  }
}
...
*/
  ...
  if ((AutoCurrentCal) && (RaceStatus == RACE_NOTSTARTED) && (digitalRead(KEY_PIN) == false)) {
  ...
  attachInterrupt(digitalPinToInterrupt(RPM_PIN), ISR_MS, RISING);


  // attach on interrup to the key read pin, so we can track pitting etc.
  attachInterrupt(digitalPinToInterrupt(KEY_PIN), ISR_KEYSTATE, CHANGE);
...
void DumpRecordSetData() {

  uint32_t ii = 0;
  uint8_t RecordType = 0;

  Serial.print("Finding first recordset address....");

  for (ii = 1; ii <= SSD.getLastRecord(); ii++) {
    SSD.gotoRecord(ii);

    //rt = SSD.getField(RecordType, frType);

    RecordType = SSD.getHeaderField(RecordType, hrType);
    //Serial.print("RecordType: ");
    //Serial.println(RecordType);
    if (RecordType == RT_HEADER) {
      // print the address
  ...
  Serial.print("Recordset finding complete");
}
...
void DumpAllDataToScreen() {
  ...
  for (i = 1; i <= LastRecord; i++) {
    ...
    if (rt == RT_HEADER) {
      ...
      Serial.print(F(": "));
      Serial.print(SSD.getHeaderField(RecordType, hrType));
      // dataset ID
      Serial.print(F(", "));
      Serial.print(SSD.getHeaderFieldName(hrID));
      Serial.print(F(": "));
      Serial.print(SSD.getHeaderField(RecordSETID, hrID));
...
void ReplayRace() {
  ...
  int TmpLap = 0, TmpDriver = 0;
  float TmpTime = 0.0, TmpVolts = 0.0, TmpAmps = 0.0, TmpTemp = 0.0, TmpEnergy = 0.0;
  float TmpSpeed = 0.0, TmpDistance = 0.0, TmpGForce = 0.0;
  int i = 0;
  int LinesRead = 0;

  const char FileName[50] = "RaceData.csv";
  ...
  ISSDCard = DataFile.open(FileName, O_RDONLY);
  ...
  delay(50);
  if (!DataFile) {
    Serial.println("Cant access file");
    while (1) {}
  }
  ...
  while (DataFile.available()) {
    ...
    if (data == '\n') {
      ...
      sscanf(line, "%lu,%f,%i,%i,%f,%f,%f,%f,%f,%f,%f",
             &TmpPoint, &TmpTime, &TmpLap, &TmpDriver,
             &TmpVolts, &TmpAmps, &TmpTemp,
             &TmpEnergy, &TmpSpeed, &TmpDistance, &TmpGForce);
      ...
      DriverID[1] = 1;
      DriverID[2] = 2;
      CarRaceTimer = TempTimer;
      DriverLaps[2] = 0;
      TargetAmps = TmpAmps;
      DriverTime[0] = TmpTime * 120;
      DriverTime[1] = 100;
      DriverTime[2] = 100;
      LapTime = 100;
      GForce = TmpGForce;
