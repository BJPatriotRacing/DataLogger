...
void SendData() {
  ...
  Data.LAPTIME = (uint16_t)(GPSDistance * 3.28084);  // data stored in ms
...
void TimeView() {
  ...
  if (GPSDistance < 32) {
    ffDriver.print(GPSDistance * 3.28084, 0);
  } else {
    ffDriver.print(0);
  }
...
void InitializeSensors() {
  ...
  digitalWrite(GPSLED_PIN, HIGH);
  GPSStatus = false;
  while (GPSUpdateTimer < 100) {  // test for 12 sec 12000
    GPSRead();
    GPSLat = GPS.location.lat();
    GPSLon = GPS.location.lng();
    GPSStatus = GPS.location.isValid();
