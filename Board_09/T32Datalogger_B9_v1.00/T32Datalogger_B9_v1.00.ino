...
#include <EEPROM.h>                // https://github.com/PaulStoffregen/EEPROM/blob/master/EEPROM.h
#include <BulletDB.h>               // https://github.com/KrisKasprzak/BulletDB
#include <EasyTransfer.h>          // https://github.com/madsci1016/Arduino-EasyTransfer/blob/master/EasyTransfer/EasyTransfer.h
#include <TinyGPSPlus.h>           // https://github.com/mikalhart/TinyGPSPlus
#include <TimeLib.h>               // https://github.com/PaulStoffregen/Time
#include <EBYTE.h>                 // https://github.com/KrisKasprzak/EBYTE
#include <FlickerFreePrint.h>      // https://github.com/KrisKasprzak/FlickerFreePrint
#include <ILI9341_t3_Menu.h>       // https://github.com/KrisKasprzak/ILI9341_t3_Menu
...
#include <PatriotRacing_Icons.h>      // custom icon file
...
#define THM_PIN A2  // thermisto measurement pin
#define AM_PIN A3   // amp sensor pin
// A4 A5 (I2C) reserved for altimiter
...
// note the ACS-770 U200 curent sensor has sensitivity of 10 mV/Amp, 0.5 volts offset at 0 amps,
...
float thVolts = 0.0f, MotorTemp = 0.0f, TempK = 0.0f;     // computed temp values
...
float iTemp = 0.0f, iPressure = 0.0f, iHumidity = 0.0f, iAltitude = 0.0f;
...
uint8_t SensorMenuOption10 = 0, SensorMenuOption11 = 0;
uint8_t GForceMenuOption1 = 0, GForceMenuOption2 = 0;
...
uint8_t frType = 0, frID = 0, frPoint = 0, frLap = 0, frDriver = 0, frVolts = 0, frAmps = 0, frMotorTemp = 0, frEnergy = 0, frRPM = 0;
...
uint8_t hrD0ID = 0, hrD1ID = 0, hrD2ID = 0, hrCarID = 0, hrMotorID = 0, hrTemp = 0, hrPressure = 0, hrHumidity = 0, hrAltitude = 0;
...
void setup() {
  ...
  // setup pin modes
  pinMode(RPM_PIN, INPUT_PULLUP);  // must have external 4k7 pullup
...
void loop() {
  ...
  if (DisplayUpdateTimer >= UPDATE_LIMIT) {
    ...
    if ((RaceStatus == RACE_INPROGRESS) && ((CarRaceTimer / 1000) >= (5400 + 300))) {
...
void ComputeData() {
  ...
  if (RaceStatus == RACE_INPROGRESS) {
    ...
    TRem = ((5400 - (CarRaceTimer / 1000.0f)) / 5400) * 100.0f;
...
void SendData() {
  ...
  Data.WARNINGS_TEMPF = (uint16_t)(Warnings << 8) | (((uint16_t)MotorTemp) & 0b0000000011111111);
  ...
  Data.GFORCE_DRIVERNO = Driver & 0b0000000000000011;
...
uint16_t RestoreRaceData() {
  ...
  // total downtime = seconds to restore
...
void CreateUserInterface() {
  ...
  SensorMenuOption1 = SensorMenu.addNI("Volt slope", VoltageSlope, 10.0, 12.0, 0.005, 3, NULL);
  SensorMenuOption2 = SensorMenu.addNI("Volt offset", VoltageOffset, 0.20, 0.40, 0.001, 3, NULL);
  SensorMenuOption3 = SensorMenu.addNI("Amp slope (20.0)", mVPerAmp, 15, 25.0, 0.01, 2);
  SensorMenuOption4 = SensorMenu.addNI("Amp offset (0.500)", VMid, .4, 0.6, 0.01, 2);
  SensorMenuOption5 = SensorMenu.addNI("Thermistor resistor", TempCF, 9000.0, 15000.0, 50, 0);
  ...
  SensorMenuOption11 = SensorMenu.addNI("Min. pulses", MinPulses, 2, 5, 1);
  ...
  while (MenuOption > 0) {
    ...
    if (digitalRead(LPin) == LOW) {
      ...
      if (val == LONG_PRESS) {
        ...
        TempCF = SensorMenu.value[SensorMenuOption5];                        // temp offset
...
void DownloadRaceData() {
  ...
  for (i = 1; i <= LastRecord; i++) {
    ...
    if (rt == RT_DATA) {
      ...
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(tPoint / 60.0, 3);
      ...
      if (tPoint < 51) {
        ...
        switch (tPoint) {
          ...
          case 12:
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(SSD.getHeaderField(iHumidity, hrHumidity));
            break;
          case 13:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(SSD.getHeaderField(iAltitude, hrAltitude));
            break;
          ...
          case 17:
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("=Y18 / Y17 * 100"));
            break;
          case 18:
            SDDataFile.write(DATA_DELIMITER);
          ...
          case 33:
            ...
            SDDataFile.print(F("=COUNTIF(D2:D10000,Y32)/60"));  // driver time
            SDDataFile.write(34);                               // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                               // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=COUNTIF(D2:D10000,Z32)/60"));  // driver time
            SDDataFile.write(34);                               // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=COUNTIF(D2:D10000,AA32)/60"));  // driver time
            SDDataFile.write(34);                                // write end " to keep comma in formula
            break;
          case 34:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                       // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(J2:J10000, D2:D10000, Y32)"));  // driver time //MAXIFS(A2:A7,B2:B7,1)
            SDDataFile.write(34);                                       // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                           // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(J2:J10000, D2:D10000, Z32)-Y35"));  // driver time
            SDDataFile.write(34);                                           // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                                // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(J2:J10000, D2:D10000, AA32)-Y35-Z35"));  // driver time
            SDDataFile.write(34);                                                // write end " to keep comma in formula
            break;
          case 35:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                       // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(C2:C10000, D2:D10000, Y32)"));  // driver time
            SDDataFile.write(34);                                       // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                           // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(C2:C10000, D2:D10000, Z32)-Y36"));  // driver time
            SDDataFile.write(34);                                           // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                                // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(C2:C10000, D2:D10000, AA32)-Y36-Z36"));  // driver time
            SDDataFile.write(34);                                                // write end " to keep comma in formula
            break;
          case 36:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                       // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(M2:M10000, D2:D10000, Y32)"));  // driver time
            SDDataFile.write(34);                                       // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                           // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(M2:M10000, D2:D10000, Z32)-Y37"));  // driver time
            SDDataFile.write(34);                                           // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                                // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(M2:M10000, D2:D10000, AA32)-Y37-Z37"));  // driver time
            SDDataFile.write(34);                                                // write end " to keep comma in formula
            break;
          case 37:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(Y37>0,Y35/Y37,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(Z37>0,Z35/Z37,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(AA37>0,AA35/AA37,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            break;
          case 38:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(Y34>0,Y35/Y34,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(Z34>0,Z35/Z34,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(AA34>0,AA35/AA34,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            break;
          case 40:
          ...
          case 44:
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("Time recorded"));
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("= MAX(B2:B10000)"));
            break;
          case 45:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("=COUNTA(A2:A10000)"));
            break;
          case 46:
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("Points recorded"));
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("=COUNTA(U2:U10000)"));
            break;
          case 47:
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("Recorded start time"));
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("=MIN(N2:N10000)"));
            break;
          case 48:
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("Recorded end time"));
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("=MAX(N2:N10000)"));
            break;
          case 49:
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("Recorded race time"));
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("=Z49-Z48"));
            break;
          case 50:
            SDDataFile.write(DATA_DELIMITER);
...
void DownloadEEPROM() {
  ...
  SDDataFile.print(F("Thermistor resistor: "));
  SDDataFile.println(TempCF);
...
void BuildFieldList() {
  ...
  hrAltitude = SSD.addHeaderField("Amp Press", &iAltitude);
  hrHumidity = SSD.addHeaderField("Amp RH", &iHumidity);
...
void InitializeSensors() {
  ...
  Display.setTextColor(C_GREEN);
  Display.print(F("FUTURE"));

  // test Motortemp sensor
  thVolts = thVolts / (BIT_CONVERSION / 3.3);
  tr2 = (thVolts * TempCF) / (3.3 - thVolts);
