/*
  ...
  a) the limit as needed #define MAX_FIELDS 25

  // in SPI.cpp (Pauls library NOT the default)
  a) In SPI.cpp at about line 1283, tp uint32_t fastio = IOMUXC_PAD_DSE(5) | IOMUXC_PAD_SPEED(2);
...
#define EXTADC_CYBORGSECONDLIMIT_PIN CH6
#define EXTADC_KEY_PIN CH7

#define EXADC_VREF 2.5f

#define TURBO_PIN 35       // manual override to unilimit current draw
...
#define BIT_CONVERSION 4095
#define EXADC_BIT_CONVERSION 65536.0f

#define SD_SPI_SPEED 30  // in MHZ
#define REFERENCE_VOLTAGE 3.3f
#define TRIGGER_AMPS 15.0f
...
int16_t EnergyID, bEnergyID, GForceXID, GForceYID, GForceZID, GForceID, CyborgAmpsID;
int16_t GraphVoltsID, GraphAmpsID, GraphLapAmpsID, GraphSpeedID, GraphMTempID, GraphCyborgInID, GraphCyborgOutID;
uint8_t Button = 0;
...
float vVolts = 0.0f, Volts = 0.0f, MinVolts = DUMMY_MAX;
float thmVolts = 0.0f, thxVolts = 0.0f, AmbTemp = 0.0f, MotorTemp = 0.0f, AuxTemp = 0.0f, TempK = 0.0f;
...
float LapAmps = 0.0, LapVolts = 0.0f, AverageAmps = 0.0f, MaxLapAmps = 19.5f, TargetAmps = 0.0f, TempTargetAmps = 0.0f;
...
uint8_t SSDMenuOption1 = 0, SSDMenuOption2 = 0, SSDMenuOption3 = 0, SSDMenuOption4 = 0, SSDMenuOption5 = 0, SSDMenuOption6 = 0;
...
uint8_t frAmbTemp = 0, frCyborgInSignal, frCyborgOutSignal, frKp = 0, frKi, frKd;

// header ID variables
uint8_t hrType = 0, hrID = 0, hrYear = 0, hrMonth = 0, hrDay = 0, hrHour = 0, hrMinute = 0, hrMSprocket = 0, hrWSprocket = 0;
uint8_t hrAltitude = 0, hrTireID = 0, hrTirePressureID = 0;
...
LTC1867 EXTADC(EXTADC_CS_PIN);  // my chip select pin
...
void setup() {
  ...
  // init the SSD chip
  // if init fails to start
  // increase delay at line 101 or so in LTC1867.h
  // maybe the PCB board traces add capacitance??
  delay(100);
  EXTADCStatus = EXTADC.init();
  //Serial.print("ADC Status ");
  //Serial.println(EXTADCStatus);
  ...
  config.window = 5000;   /* in seconds, 0->128 */
  config.timeout = 10000; /* in seconds, 0->128 */
...
void loop() {
  ...
  if (EnableCyborg) {
    ...
    CyborgLimitVolts = EXTADC.analogRead(EXTADC_CYBORGFIRSTLIMIT_PIN) / (EXADC_BIT_CONVERSION / EXADC_VREF);
    CyborgLimitVolts = FloatMap(CyborgLimitVolts, 0.0, 2.5, 14.0, 24.0);
    CyborgLimitVolts = (CyborgLimitVolts * 10.0) / 10.0f;
    if (abs(CyborgLimitVolts - CyborgFirstLimit) > ChangeLimit) {
      ChangeLimit = 0.1f;
      CyborgFirstLimit = CyborgLimitVolts;
    ...
    CyborgLimitVolts = EXTADC.analogRead(EXTADC_CYBORGSECONDLIMIT_PIN) / (EXADC_BIT_CONVERSION / EXADC_VREF);
    CyborgLimitVolts = FloatMap(CyborgLimitVolts, 0.0, 2.5, 24.0, 45.0);
    CyborgLimitVolts = (CyborgLimitVolts * 10.0) / 10.0f;
    if (abs(CyborgLimitVolts - CyborgSecondLimit) > ChangeLimit) {
      ChangeLimit = 0.1f;
      CyborgSecondLimit = CyborgLimitVolts;
      if (DriverChangeScreen > 3000) {
        RedrawHeader = true;
      }
      CyborgLimitChange = true;
    ...
    if (CyborgTimer > CyborgUpdateTime) {
      ...
      Input = CyborgAmps;
      CyborgPID.Compute();  // really I think this needs to be in main loop...
  ...
  if (DisplayUpdateTimer >= UPDATE_LIMIT) {
    ...
    if (EXTADC.analogRead(EXTADC_KEY_PIN) > 3000) {
      KeyState = HIGH;
    } else {
      KeyState = LOW;
    }
    ...
    if ((Amps >= TRIGGER_AMPS) && (AllowDriverChange) && (KeyState == HIGH)) {
      AllowDriverChange = false;
      CheckIfStarting();
    }
...
void ComputeData() {
  ...
  // Volts = (vVolts + VoltageOffset) * VoltageSlope;
...
void SendData() {
  ...
  Data.D0TIME_ALTITUDE = (((uint16_t)(DriverTime[0] / 1000) << 4) | (((uint16_t)GPSAltitude >> 8) & 0b0000000000001111));  // msb
  Data.D1TIME_ALTITUDE = (((uint16_t)(DriverTime[1] / 1000) << 4) | (((uint16_t)GPSAltitude >> 4) & 0b0000000000001111));
  Data.D2TIME_ALTITUDE = (((uint16_t)(DriverTime[2] / 1000) << 4) | (((uint16_t)GPSAltitude) & 0b0000000000001111));
...
void CreateUserInterface() {
  ...
  SSDMenuOption2 = SSDMenu.addNI("Quick ERASE ");
  ...
  while (MenuOption > 0) {
    ...
    if (ButtonPressed == C_BUTTON) {
      ...
      if (MenuOption == SSDMenuOption1) {
        ...
        config.window = 120000;  /* in seconds, 0->128 */
        config.timeout = 500000; /* in seconds, 0->128 */
        config.callback = WDTCallback;
        WDT.begin(config);
      ...
      if (MenuOption == SSDMenuOption2) {
        UsedBlocks = SSD.getUsedBlocks();
        Display.fillRoundRect(13, 182, 4, 36, 2, C_GREEN);
        for (i = 0; i <= UsedBlocks; i++) {
          Display.fillRoundRect(13, 182, (160 * i) / UsedBlocks, 36, 2, C_GREEN);
          SSD.eraseBlock(i);
        }
        DrawInfoScreen();
      }
...
void DownloadRaceData(uint32_t Count) {
  ...
  for (i = 1; i <= LastRecord; i++) {
    ...
    if (rt == RT_DATA) {
      ...
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(SSD.getField(GPSAltitude, frAlt), 1);
      SDDataFile.write(DATA_DELIMITER);
      ...
      SDDataFile.write(DATA_DELIMITER);
      //SDDataFile.print(SSD.getField(Kp, frKp), 4);  //
      SDDataFile.write(DATA_DELIMITER);
      //SDDataFile.print(SSD.getField(Ki, frKi), 4);  //
      SDDataFile.write(DATA_DELIMITER);
      //SDDataFile.print(SSD.getField(Kd, frKd), 4);  //
      ...
      if (tPoint <= 55) {
        ...
        switch (tPoint) {
          ...
          case 17:
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.print(F("=AH18 / AH17 * 100"));
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(DATA_DELIMITER);
          ...
          case 34:
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                 // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=COUNTIF(D2:D13000,AI33)/120"));  // driver time
            SDDataFile.write(34);                                 // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                 // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=COUNTIF(D2:D13000,AJ33)/120"));  // driver time
            SDDataFile.write(34);                                 // write end " to keep comma in formula
            break;
          case 35:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                        // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(K2:K13000, D2:D13000, AH33)"));  // driver time //MAXIFS(A2:A7,B2:B7,1)
            SDDataFile.write(34);                                        // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                             // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(K2:K13000, D2:D13000, AI33)-AH36"));  // driver time
            SDDataFile.write(34);                                             // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                                  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(K2:K13000, D2:D13000, AG33)-AH36-AI36"));  // driver time
            SDDataFile.write(34);                                                  // write end " to keep comma in formula
            break;
          case 36:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                        // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(C2:C13000, D2:D13000, AH33)"));  // driver time
            SDDataFile.write(34);                                        // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                             // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(C2:C13000, D2:D13000, AI33)-AH37"));  // driver time
            SDDataFile.write(34);                                             // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                                  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(C2:C13000, D2:D13000, AJ33)-AH37-AI37"));  // driver time
            SDDataFile.write(34);                                                  // write end " to keep comma in formula
            break;
          case 37:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                        // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(N2:N13000, D2:D13000, AH33)"));  // driver time
            SDDataFile.write(34);                                        // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                             // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(N2:N13000, D2:D13000, AI33)-AH38"));  // driver time
            SDDataFile.write(34);                                             // write end " to keep comma in formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);                                                  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=MAXIFS(N2:N13000, D2:D13000, AJ33)-AH38-AI38"));  // driver time
            SDDataFile.write(34);                                                  // write end " to keep comma in formula
            break;
          case 38:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(AH37>0,AH36/AH38,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(AI37>0,AI36/AI38,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(AJ37>0,AJ36/AJ38,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            break;
          case 39:
            SDDataFile.write(DATA_DELIMITER);
            ...
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(AH35>0,AH36/AH35,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(AI35>0,AI36/AI35,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.write(DATA_DELIMITER);
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            SDDataFile.print(F("=IF(AJ35>0,AJ36/AJ35,0)"));
            SDDataFile.write(34);  // write a " that way the comma in the formula will stay in the formula
            break;
          case 41:
...
void DownloadEEPROM() {
  ...
  SDDataFile.print(F("Cyborg Max Current: "));
  ...
  SDDataFile.println(TRIGGER_AMPS);
...
void BuildFieldList() {
  ...
  hrTemp = SSD.addHeaderField(&AmbTemp);                 // "Amb Temp"
...
void InitializeSensors() {
  ...
  if (EXTADCStatus) {
    Display.setTextColor(C_GREEN);
    Display.print(F(" / ADC OK"));
  } else {
    Display.setTextColor(C_RED);
    Display.print(F(" / ADC FAIL"));
  }
  ...
  // get current draw
  // aVolts = EXTADC.analogRead(EXTADC_AM_PIN);
  ...
  CyborgLimitVolts = EXTADC.analogRead(EXTADC_CYBORGFIRSTLIMIT_PIN) / (EXADC_BIT_CONVERSION / EXADC_VREF);
  CyborgFirstLimit = FloatMap(CyborgLimitVolts, 0.0, 3.3, 14.0, 24.0);

  CyborgLimitVolts = EXTADC.analogRead(EXTADC_CYBORGSECONDLIMIT_PIN) / (EXADC_BIT_CONVERSION / EXADC_VREF);
  CyborgSecondLimit = FloatMap(CyborgLimitVolts, 0.0, 3.3, 24.0, 45.0);
}
...
void meansensors() {
  ...
  while (i < (buffersize + 101)) {
    ...
    i++;
    delay(2);  // was 2 Needed so we don't get repeated measures
  }
}
...
void CalibrateAccererometer() {
  ...
  while (1) {
    ...
    if ((Found == 6) || (Tries >= 50) || digitalRead(R_PIN) || digitalRead(L_PIN)) {
      break;
    }
  }
  if (Found != 6) {
    ...
    GForceMenu.value[GForceMenuOption2] = (int16_t)AccelCalX;
    GForceMenu.value[GForceMenuOption3] = (int16_t)AccelCalY;
    GForceMenu.value[GForceMenuOption4] = (int16_t)AccelCalZ;
