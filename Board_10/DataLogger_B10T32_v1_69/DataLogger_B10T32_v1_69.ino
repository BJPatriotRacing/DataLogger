...
#include <MS5837.h>     // altimiter chip https://github.com/bluerobotics/BlueRobotics_MS5837_Library
...
#define CODE_VERSION "10.32.069.01"
...
#define CYBORG_FL_LOW 4.0f
#define CYBORG_FL_HIGH 30.0f

/*-------------------*/
// Program Variables
/*-------------------*/
float EXADC_VREF = 3.28f;
uint16_t DATA_X = 305;  // left justify would be 15;
bool DrawGraph = true;
uint16_t EnergyPoints[100];

// 0 to 526
uint16_t BLEnergy[527] = { 0, 1, 2, 3, 5, 6, 7, 8, 10, 11, 12, 13, 14, 16, 17, 18, 19, 21, 22, 23, 24, 26, 27, 28, 29, 30, 31, 32, 34, 35, 36, 37, 39, 40, 41, 42, 44, 45, 46, 47, 48, 50, 51,
                           ...
                           454, 455, 456, 458, 459, 460, 461, 462, 463, 464, 465, 466, 467, 468, 470, 471, 472, 472, 474, 475, 476, 477, 478, 479, 480, 481, 482, 483, 484, 486, 487, 488, 489, 490, 491, 492, 493, 494,
...
uint8_t CyborgInput = 0;
bool CyborgTurbo = false;
bool CyborgActive = false;
float CyborgThresholdVoltsOn = 3.1f;
float CyborgThresholdVoltsOff = 0.8f;
double Setpoint = 0.0, Input = 0.0, Output = 0.0;
double Kp = 0.5, Ki = 155.0, Kd = 0.1;
int CyborgXPoint = 0;
...
//Setup Variables
uint8_t RadioUpdate = 1;     // index to char array
...
uint8_t KeyState = LOW, OldKeyState = HIGH, DisplayOldKeyState = LOW, KeyStatus = KEYSTATUS_TURNED_ON;

uint8_t MaxG = 0;
float LapSpeed = 0.0f;
float MaxLapGForceY = 0.0f;
float GForceYLapMaxL = 0.0f, GForceYLapMaxR = 0.0f, TempGForceYLapMaxL = 0.0f, TempGForceYLapMaxR = 0.0f;
...
uint8_t AirDataRate = 0, oAirDataRate = 0;
uint8_t RadioPower = 0, oRadioPower = 0;
uint8_t RadioChannel = 0, oRadioChannel = 0;
...
uint8_t MainMenuID1 = 0, MainMenuID2 = 0, MainMenuID3 = 0;
uint8_t MainMenuID4 = 0, MainMenuID5 = 0, MainMenuID6 = 0, MainMenuID7 = 0;
uint8_t MainMenuID8 = 0, MainMenuID9 = 0;

uint8_t RaceMenuID1 = 0, RaceMenuID2 = 0, RaceMenuID3 = 0, RaceMenuID4 = 0;
uint8_t RaceMenuID5 = 0, RaceMenuID6 = 0, RaceMenuID7 = 0, RaceMenuID8 = 0;
uint8_t RaceMenuID9 = 0, RaceMenuID10 = 0, RaceMenuID12, RaceMenuID13;

uint8_t SettingsMenuID1 = 0, SettingsMenuID2 = 0, SettingsMenuID3 = 0, SettingsMenuID4 = 0;
uint8_t SettingsMenuID5 = 0, SettingsMenuID6 = 0;

uint8_t WirelessMenuID1 = 0, WirelessMenuID2 = 0, WirelessMenuID3 = 0, WirelessMenuID4 = 0;
uint8_t WirelessMenuID5 = 0, WirelessMenuID6 = 0, WirelessMenuID7 = 0;

uint8_t SensorMenuID1 = 0, SensorMenuID2 = 0, SensorMenuID3 = 0, SensorMenuID4 = 0;
uint8_t SensorMenuID5 = 0, SensorMenuID6 = 0, SensorMenuID7 = 0;
uint8_t SensorMenuID9 = 0, SensorMenuID10 = 0, SensorMenuID11 = 0, SensorMenuID12 = 0;

uint8_t GForceMenuID1 = 0, GForceMenuID2 = 0, GForceMenuID3 = 0, GForceMenuID4 = 0;
uint8_t GForceMenuID5 = 0, GForceMenuID6 = 0, GForceMenuID7 = 0, GForceMenuID8 = 0;

uint8_t ClockMenuID1 = 0, ClockMenuID2 = 0, ClockMenuID3 = 0, ClockMenuID4 = 0;
uint8_t ClockMenuID5 = 0, ClockMenuID6 = 0;
...
uint8_t CyborgMenuID9 = 0, CyborgMenuID10 = 0, CyborgMenuID11 = 0, CyborgMenuID12 = 0;
...
uint8_t frAmbTemp = 0, frCyborgInSignal = 0, frCyborgOutSignal = 0, frAirSpeed = 0, frAirOffset = 0;
...
// structure for the wireless data tansmiFssion
Transceiver Data;
...
ILI9341_FlickerFreePrint ffOutput(&Display, C_WHITE, C_BLACK, JUSTIFY_LEFT);
...
void loop() {
  ...
  if (EnableCyborg) {
    ...
    if (CyborgTimer > CyborgUpdateTime) {
      ...
      3.25 / 4.0 = 0.8125 so low becomes 1.0 x 0.82 so input signal is now 0.82 to 3.25
      ...
      for PID to work we need to get amps, compute PID output on every loop pass. These adjustments
      need to be real-time (hence it cannot be in some averaging or timed routine)
      setpoint can be and is in some averaging routine
      */
      CyborgFL = CyborgFL / CyborgCounter;
      CyborgFL = CyborgFL / (EXADC_BIT_CONVERSION / EXADC_VREF);

      // if the driver input control is use the pot to set the total energy so we have control over AME aggression
      // if the switch is off then the we disable AME and the pot is used to set the current limit a.k.a race limit a.k.a FirstLimit

      if (EXTADC.analogRead(EXTADC_ENABLE_AME_PIN) > DIGITAL_ON_LIMIT) {
        // round to nearest 5 wHr
        TotalEnergy = (uint16_t)FloatMap(CyborgFL, 0.0, 3.3, 500, 800);
        TotalEnergy = (uint16_t)(TotalEnergy / 5.0f) * 5.0f;
      } else {

        // round to nearest .1 amps
        CyborgFirstLimit = FloatMap(CyborgFL, 0.0, 3.3, CYBORG_FL_LOW, CYBORG_FL_HIGH);
        CyborgFirstLimit = (uint16_t)(CyborgFirstLimit * 10.0f) / 10.0f;
      }

      // in loop() we measure energy do linear regression get a prediction which is reference only. AME is computed by
      // simply looking at available energy the bias cyborg first limit accordingly
      // bias compensation is set in ComputeAME()
    ...
    // pass in  Input - which is the measured current
    // pass in  Setpoint is either 1) first limit 2) first limit with bias or 3) second limit (turbo)
    // pass out Output which is the PWM signal (800 to 4096)
    ...
    if (ThrottleInputVolts >= CyborgThresholdVoltsOn) {
      CyborgActive = true;
      CyborgOutputPWM = Output;
      // if we consider above x fully on, then clamp the display to 100%
      CyborgInSignal = 100.0f;
    } else {
      CyborgActive = false;
      CyborgOutputPWM = ThrottleInputBits;
    }
    if (ThrottleInputVolts < CyborgThresholdVoltsOff) {
      CyborgActive = false;
      CyborgOutputPWM = 0;
      // if we consider below y fully off, then clamp the display to 0%
      CyborgInSignal = 0.0f;
    }
    ...
    CyborgInSignal = FloatMap(CyborgInSignal, 18.0f, 100.0f, 0.0f, 100.0f);

    CyborgOutSignal = (CyborgOutputPWM * 100) / BIT_CONVERSION;
    CyborgOutSignal = map(CyborgOutSignal, 18, 100, 0, 100);
    if (CarSpeed == 0) {
      CyborgOutSignal = 0;
    }
  }
  ...
  // for testing we're putting speed calcs in the Cyborg section
  ...
  if (DisplayUpdateTimer >= UPDATE_LIMIT) {
    ...
    // if the RTC battey died, we should get a date/time but will out of date
    ...
    // set states for pit time writing
    ...
    if (RaceStatus == RACE_INPROGRESS) {
      ...
      if (KeyState == HIGH) {  // do not bias CyborgFirstLimit if key is off
        // in this function we always compute predicted energy, but we do not use it to compute AME
        ComputePrediction();
        // we only adjust CyborgFirst limit if AME is active (meaning the driver control pad switch is ON)

        if (EXTADC.analogRead(EXTADC_ENABLE_AME_PIN) > DIGITAL_ON_LIMIT) {
          ComputeAME();
        }
      } else {
        // assume we are in pit
        // key off, predicted energy is simple...it's what Energy currently is
        PredictedEnergy = Energy;
        // and reset CyborgFirstLimit
        EEPROM.get(485, CyborgFirstLimit);
      }
    ...
    if (RadioUpdate != 0) {
      if (RadioUpdateTimer >= (RadioUpdate * 1000)) {
        RadioUpdateTimer = 0;
        // this is very weird, but the EBYTE introduces enough noise to throw the VREF off slightly where delta = 0.06 VDC
        // probably should have put and external VREF on the ADC. A simple change brings the delta to 0.002 VDC
        EXADC_VREF = 3.283f;
        SendData();
      }
    }
...
void ComputeData() {
  ...
  thmVolts = EXTADC.analogRead(EXTADC_THM_PIN);
...
void ComputePrediction() {
  ...
  if (PredictionArrayCounter < PREDICT_SAMPLES) {
    ...
    PredictedEnergy = PredictedEnergy - (PredictionCompensation * (10800.0f - (float)Point)) / 10800.0f;
...
void ComputeAME() {
  ...
  if (Point < (527 * 20)) {
    if (Point % 20 == 0) {
      // need to scale baseline energy to total energy before we compare
      // give a +/- 1% buffer to actual energy to account for some tolerance and minimize jumping
      if ((1.01f * Energy) > (BLEnergy[Point / 20] * ((float)BLEnergy[526]))) {
        // energy exceeds what baseline is, so reduced current bias
        CyborgFirstLimit = CyborgFirstLimit - CurrentBias;
      } else if ((0.99f * Energy) < (BLEnergy[Point / 20] * ((float)BLEnergy[526]))) {
        // we have more energy than we are useing so increase CyborgFirstLimit
        CyborgFirstLimit = CyborgFirstLimit + CurrentBias;
      }
    }
  }
...
void SendData() {
  ...
  //Serial.write((uint8_t *)&Data, PacketSize);
  SmartDelay(20);
  // ESerial.flush();
}
...
void GetParameters() {
  ...
  // could be due to corrupted eeprom data or added parameter and eeprom has some old data
    ...
    if (RestoreEEPROM) {
      ...
      */
      ...
      CyborgThresholdVoltsOn = 3.2f;
      EEPROM.put(385, CyborgThresholdVoltsOn);
      CyborgThresholdVoltsOff = 0.8f;
      EEPROM.put(390, CyborgThresholdVoltsOff);
      RaceMonth = 0;
      EEPROM.put(400, RaceMonth);
  ...
  */
  ...
  EEPROM.get(385, CyborgThresholdVoltsOn);
  EEPROM.get(390, CyborgThresholdVoltsOff);
  ...
  Serial.print(F("CyborgThresholdVoltsOn: "));
  Serial.println(CyborgThresholdVoltsOn);
...
void SpeedView() {
  ...
  ffLapSpeed.print(Setpoint, 1);

  Display.setCursor(10, LADATA_Y);
  ffOutput.setTextColor(C_WHITE, C_DKGREY);
  ffOutput.print(CyborgOutSignal);
}
...
void CYBORGView() {
  ...
  if (CyborgOutSignal <= ThrottlePercent) {
    Display.fillRect(10, 50, (CyborgOutSignal * 3), 60, C_BLUE);
    Display.fillRect((ThrottlePercent * 3) + 10, 50, 10, 60, CyborgColor);
    Display.fillRect((CyborgOutSignal * 3) + 10, 50, (ThrottlePercent - CyborgOutSignal) * 3, 60, C_DKBLUE);
    Display.fillRect((ThrottlePercent * 3) + 10 + 10, 50, 314 - (ThrottlePercent * 3) - 10, 60, C_DKBLUE);
  } else {
    Display.fillRect(10, 50, (ThrottlePercent)*3, 60, C_BLUE);
    Display.fillRect((ThrottlePercent * 3) + 10, 50, 10, 60, CyborgColor);
    Display.fillRect((ThrottlePercent * 3) + 10 + 10, 50, (CyborgOutSignal - ThrottlePercent) * 3, 60, C_BLUE);
    Display.fillRect((CyborgOutSignal * 3) + 16, 50, 314 - (CyborgOutSignal * 3) - 10, 60, C_DKBLUE);
  }

  if (EXTADC.analogRead(EXTADC_ENABLE_AME_PIN) > DIGITAL_ON_LIMIT) {

    Display.setFont(FONT_24BI);
    // AME on left side is TotalEnergy (set by driver)
    Display.setCursor(70, 200);
    ffCyborgSetpoint.setTextColor(C_WHITE, C_DKGREY);
    ffCyborgSetpoint.print(TotalEnergy);
    // right side is predicted energy
    Display.setCursor(310, 200);
    ffCyborgInput.setTextColor(C_WHITE, C_DKGREY);
    ffCyborgInput.print(PredictedEnergy, 0);
  } else {

    Display.setFont(FONT_24BI);
    // AME is off so left side is CyborgFirstLimit
    Display.setCursor(70, 200);
    ffCyborgSetpoint.setTextColor(C_WHITE, C_DKGREY);
  ...
  // stuff in center bottom (predicted consumption in percent)
  Display.setFont(FONT_48BINO);
  Display.setCursor(237, 180);
...
void AmpsView() {
  if (RedrawHeader) {
    ...
    LapAverageBar.setSectionSize(WARNING_LAPAMP - 0.5f, WARNING_LAPAMP + 0.5f);
    LapAverageBar.setScale(WARNING_LAPAMP - 5.0f, WARNING_LAPAMP + 5.0f, 1);
    RedrawHeader = false;
  }
...
void VoltsView() {
  if (RedrawHeader) {
    ...
    Display.setCursor(5, 180);
    Display.print(F("Lap Average"));
    LapAverageBar.setSectionSize(WARNING_BATTERY, WARNING_BATTERY + 1.0f);
  ...
  //Bottom Info
  if ((LapVolts + 1.0f) < WARNING_BATTERY) {
...
void CreateUserInterface() {
  ...
  RaceMenuID9 = RaceMenu.addNI("Tire pressure [psi]", TirePressure, 70, 200, 5);
  ...
  SettingsMenuID5 = SettingsMenu.addNI("Start/Driver trigger [a]", TriggerAmps, 10, 90, 5, 0);
  SettingsMenuID1 = SettingsMenu.addNI("Motor", MotorID, 0, sizeof(MotorText) / sizeof(MotorText[0]), 1, 0, MotorText);
  SettingsMenuID2 = SettingsMenu.addNI("Screen button location", Orientation, 0, sizeof(OrientationText) / sizeof(OrientationText[0]), 1, 0, OrientationText);
  ...
  WirelessMenuID5 = WirelessMenu.addNI("Altitude correction [ft]", AltCorrection, -400, 400, 5, 0);
  ...
  SensorMenuID2 = SensorMenu.addNI("Volt offset (0.32)", VoltageOffset, 0.20, 0.40, 0.001, 3, NULL);
  SensorMenuID3 = SensorMenu.addNI("Amp slope (20 | 40)", mVPerAmp, 10, 90.0, 0.01, 2);
  SensorMenuID4 = SensorMenu.addNI("Amp offset (0.5 | 0.6)", VMid, 0.4, 0.8, 0.001, 3);
  ...
  CyborgMenuID1 = CyborgMenu.addNI("Race current [A]", CyborgFirstLimit, CYBORG_FL_LOW, CYBORG_FL_HIGH, 0.1, 1);
  CyborgMenuID8 = CyborgMenu.addNI("Turbo current [A]", CyborgSecondLimit, 10.0, 40.0, 0.1, 1);
  CyborgMenuID10 = CyborgMenu.addNI("Cyborg full off at [V]", CyborgThresholdVoltsOff, 0, 2.0, 0.1, 1);
  CyborgMenuID7 = CyborgMenu.addNI("Cyborg full on at [V]", CyborgThresholdVoltsOn, 2.5, 3.3, 0.1, 1);
  CyborgMenuID3 = CyborgMenu.addNI("Kp (rise time)", Kp, 0, MAX_KP, .1, 1);
  CyborgMenuID4 = CyborgMenu.addNI("Ki (converge rate)", Ki, 0, MAX_KI, 1, 0);
  CyborgMenuID5 = CyborgMenu.addNI("Kd (future prediction)", Kd, 0, MAX_KD, .1, 1);
  CyborgMenuID6 = CyborgMenu.addNI("PID update time [ms]", CyborgUpdateTime, 100, 1000, 50, 0);
  CyborgMenuID12 = CyborgMenu.addNI("Cyborg Setpoint", CyborgInput, 0, sizeof(CyborgInputText) / sizeof(CyborgInputText[0]), 1, 0, CyborgInputText);

  // ame stuff
  CyborgMenuID9 = CyborgMenu.addNI("Prediction comp [A]", PredictionCompensation, 10, 180, 1, 0);
  CyborgMenuID11 = CyborgMenu.addNI("Current Bias [A]", CurrentBias, .01, .5, 0.01, 2);
  ...
  MotorTempG.init(10, 181, 40, 105, 40, 160, 20, "Motor", C_WHITE, back_color, C_MAGENTA, C_DKMAGENTA, back_color, FONT_14, FONT_14);
  ...
  AuxTempG.init(110, 181, 40, 105, 40, 140, 20, "Auxiliary", C_WHITE, back_color, C_YELLOW, C_DKYELLOW, back_color, FONT_14, FONT_14);
  ...
  AmbTempG.init(220, 181, 40, 105, 40, 100, 10, "Ambient", C_WHITE, back_color, C_CYAN, C_DKCYAN, back_color, FONT_14, FONT_14);
  ...
  TRemG.init(10, 235, 45, 160, 0, 100, 1, "Time", C_WHITE, back_color, C_CYAN, C_DKCYAN, back_color, FONT_14, FONT_14);
  ...
  ERemG.init(70, 235, 45, 160, 0, 100, 1, "Energy", C_WHITE, back_color, C_CYAN, C_DKCYAN, back_color, FONT_14, FONT_14);
...
void ProcessRaceMenu() {
  ...
  TirePressure = (uint8_t)RaceMenu.value[RaceMenuID9];
...
void ProcessWirelessMenu() {
  ...
  // reset the menu data
  WirelessMenu.SetItemValue(WirelessMenuID2, RadioChannel);
  WirelessMenu.SetItemValue(WirelessMenuID3, AirDataRate);
  WirelessMenu.SetItemValue(WirelessMenuID4, RadioPower);
  ...
  while (MenuOption > 0) {
    ...
    if ((millis() - caltime) > 1000) {
      ...
      Display.print(F("Noise/stength [db]"));
      ...
      Display.print(Radio.readRSSIAmbientNoise());
      Display.print(F(" / "));
      Display.print(Radio.readRSSISignalStrength());
...
void ProcessCyborgMenu() {
  ...
  CyborgThresholdVoltsOff = CyborgMenu.value[CyborgMenuID10];
  CyborgThresholdVoltsOn = CyborgMenu.value[CyborgMenuID7];
  PredictionCompensation = CyborgMenu.value[CyborgMenuID9];
  CurrentBias = CyborgMenu.value[CyborgMenuID11];
  CyborgInput = (uint8_t)CyborgMenu.value[CyborgMenuID12];
...
void DownloadEEPROM() {
  ...
  SDDataFile.print(F("Current Bias [A]: "));
  SDDataFile.println(CurrentBias, 2);

  // make sure we get the saved one and not any bias applied by Cyborg

  SDDataFile.print(F("Cyborg 1st current range: "));
  SDDataFile.print(CYBORG_FL_LOW);
  SDDataFile.print(F(" / "));
  SDDataFile.println(CYBORG_FL_HIGH);
  EEPROM.get(485, CyborgFirstLimit);
  ...
  SDDataFile.print(F("Cyborg Off Threshold: "));
  SDDataFile.println(CyborgThresholdVoltsOff);
  SDDataFile.print(F("Cyborg On Threshold: "));
  SDDataFile.println(CyborgThresholdVoltsOn);
