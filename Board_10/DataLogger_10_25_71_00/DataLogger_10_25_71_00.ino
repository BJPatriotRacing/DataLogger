...
#define CODE_VERSION "10.25.71.01"
...
uint8_t TirePressure = 115;  // input for tire pressure
...
uint8_t AirDataRate = 0;
uint8_t RadioPower = 0;
uint8_t RadioChannel = 0;
uint8_t RadioAddressL = 0, RadioAddressH = 0;
...
uint16_t LapTime = 0, LastLapTime = 0;
int16_t TimeDelta = 0;
...
uint8_t RaceMenuID9 = 0, RaceMenuID10 = 0, RaceMenuID12 = 0, RaceMenuID13 = 0;
...
*/
  ...
  if (EnableCyborg) {
    ...
    CyborgOutSignal = map(CyborgOutputPWM, 800, 4096, 0, 100);
  }
  ...
  if (DisplayUpdateTimer >= UPDATE_LIMIT) {
    ...
    if ((abs(GPS.date.day() - day()) > 3) || (abs(GPS.time.minute() - minute()) > 5)) {
      // waiting for GPS
...
void ComputeAME() {
  ...
  if (Point < (527 * 20)) {
    if (Point % 20 == 0) {
      ...
      if ((1.01f * Energy) > ((float)BLEnergy[Point / 20] * (TotalEnergy / (float)BLEnergy[526]))) {
        // energy exceeds what baseline is, so reduced current bias
        CyborgFirstLimit = CyborgFirstLimit - CurrentBias;
      } else if ((0.99f * Energy) < ((float)BLEnergy[Point / 20] * (TotalEnergy / (float)BLEnergy[526]))) {
        // we have more energy than we are useing so increase CyborgFirstLimit
        CyborgFirstLimit = CyborgFirstLimit + CurrentBias;
      }

      if (CyborgFirstLimit < CYBORG_FL_LOW) {
        CyborgFirstLimit = CYBORG_FL_LOW;
      }
    }
    if (CyborgFirstLimit > CYBORG_FL_HIGH) {
      CyborgFirstLimit = CYBORG_FL_HIGH;
    }
  }
}
...
void CYBORGView() {
  ...
  if (EXTADC.analogRead(EXTADC_ENABLE_AME_PIN) > DIGITAL_ON_LIMIT) {
    ...
    Display.setFont(FONT_16B);
    ffACalX.setTextColor(C_WHITE, C_DKGREY);
    Display.setCursor(5, 175);
    ffACalX.print("Energy");
...
void ProcessWirelessMenu() {
  ...
  uint8_t oRadioPower = 0, oAirDataRate = 0, oRadioChannel = 0, oRadioAddressL = 0, oRadioAddressH = 0;
  MenuOption = 1;
  ResetEBYTE = 0;
  ...
  // save to old for comparison later
  oRadioChannel = RadioChannel;
  oAirDataRate = AirDataRate;
  oRadioPower = RadioPower;
  oRadioAddressL = RadioAddressL;
  oRadioAddressH = RadioAddressH;
  ...
  WirelessMenu.SetItemValue(WirelessMenuID8, RadioAddressL);
  WirelessMenu.SetItemValue(WirelessMenuID9, RadioAddressH);
  ...
  if ((oAirDataRate != AirDataRate) || (oRadioChannel != RadioChannel) || (oRadioPower != RadioPower) || (oRadioAddressL != RadioAddressL) || (oRadioAddressH != RadioAddressH)) {
    Radio.setRSSIAmbientNoise(false);
    Radio.setRSSISignalStrength(false);
    Radio.setPacketSize(SUB_64BYTES);
    Radio.saveParameters(EBYTE_WRITE_PERMANENT);
