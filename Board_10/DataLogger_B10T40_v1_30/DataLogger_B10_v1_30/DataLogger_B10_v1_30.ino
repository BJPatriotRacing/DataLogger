...
#define CODE_VERSION "10.32.061.11"
...
#define EXTADC_CYBORGCURRENTBIAS_PIN CH6
#define EXTADC_VM_PIN CH7

// manual override to unilimit current draw
#define MAX_KP 100
#define MAX_KI 1000
#define MAX_KD 100
...
// reuse pin 8 (GPS Rx since we don't send data to it) for LED indicating GPS success and lap
#define LAPLED_PIN 8
#define DCS_PIN 9
#define SDCS_PIN 10
// 11-12-13 are SPI

#define AUX_14_PIN 14
#define AUX_15_PIN 15
...
int8_t EnergyID, bEnergyID, GForceID, CyborgAmpsID, CyborgVoltsID;
int8_t GraphVoltsID, GraphAmpsID, GraphLapAmpsID, GraphSpeedID, GraphMTempID, GraphAltitudeID;
uint8_t GraphCyborgInID, GraphCyborgOutID, GraphPredictedID;
uint8_t Button = 0;
...
uint8_t AveragingTypeID = 0;
float AdjustLimit = 0.0f;
...
double OldKp = 0.5f, OldKi = 155.0f, OldKd = 0.1f;
int CyborgXPoint = 0;
...
uint8_t SettingsMenuOption1 = 0, SettingsMenuOption2 = 0, SettingsMenuOption3 = 0, SettingsMenuOption4 = 0, SettingsMenuOption5 = 0, SettingsMenuOption6 = 0;
uint8_t SettingsMenuOption7 = 0, SettingsMenuOption8 = 0, SettingsMenuOption9 = 0;
...
uint8_t ClockMenuOption1 = 0, ClockMenuOption2 = 0, ClockMenuOption3 = 0, ClockMenuOption4 = 0, ClockMenuOption5 = 0, ClockMenuOption6 = 0;
...
uint8_t CyborgMenuOption7 = 0, CyborgMenuOption8 = 0, CyborgMenuOption9 = 0, CyborgMenuOption10 = 0;
uint8_t CyborgMenuOption11 = 0, CyborgMenuOption12 = 0, CyborgMenuOption13 = 0, CyborgMenuOption14 = 0;
...
uint8_t frAmbTemp = 0, frCyborgInSignal = 0, frCyborgOutSignal = 0, frKp = 0, frKi = 0, frKd = 0;
...
FlickerFreePrint<ILI9341_t3> ffDriverLapTime(&Display, C_WHITE, C_BLACK);
FlickerFreePrint<ILI9341_t3> ffDriverTime(&Display, C_WHITE, C_BLACK);
FlickerFreePrint<ILI9341_t3> ffTimeDelta(&Display, C_WHITE, C_BLACK);
...
FlickerFreePrint<ILI9341_t3> ffCYBORGKi(&Display, C_WHITE, C_BLACK);
...
FlickerFreePrint<ILI9341_t3> ffGForceY(&Display, C_WHITE, C_BLACK);
...
FlickerFreePrint<ILI9341_t3> ffPredict(&Display, C_WHITE, C_BLACK);
...
elapsedMillis PIDTuningTimer = 0;
...
// graph for cyborg tuning
CGraph CYBORGPIDTuneG(&Display, GRAPH_X, 210, GRAPH_W, 120, 0, 240, 60, 0, 30, 5);
...
void loop() {
  ...
  if (EnableCyborg) {
    ...
    AUX_14 = AUX_14 + analogRead(AUX_14_PIN);
    AUX_15 = AUX_15 + analogRead(AUX_15_PIN);
    ...
    if (CyborgTimer > CyborgUpdateTime) {
      ...
      AUX_14 = AUX_14 / CyborgCounter;
      AUX_14 = AUX_14 / (BIT_CONVERSION / REFERENCE_VOLTAGE);
      AUX_15 = AUX_15 / CyborgCounter;
      AUX_15 = AUX_15 / (BIT_CONVERSION / REFERENCE_VOLTAGE);

      if (EnablePIDTuning) {
        if (AUX_15 > 3.2) {  // switch connected to Vcc but give some leaway
          GettingKd = true;
          Kd = FloatMap(AUX_14, 0.1, 3.3f, 0.0f, MAX_KD);  // force near zero to zero
        } else {
          GettingKd = false;
          Ki = FloatMap(AUX_14, 0.1, 3.3f, 0.0f, MAX_KI);  // force near zero to zero
          Kp = FloatMap(AUX_15, 0.1, 3.3f, 0.0f, MAX_KP);  // force near zero to zero
        }

        // minize value bouncing
        Kp = (uint16_t)(Kp * 10.0f) / 10.0f;
        Ki = (uint16_t)Ki;
        Kd = (uint16_t)(Kd * 10.0f) / 10.0f;

        // if change update EEPROM
        if (abs(Kp - OldKp) > AdjustLimit) {
          PIDTuningTimer = 0;
          OldKp = Kp;
          EEPROM.put(500, Kp);
          if (DisplayID != 9) {
        ...
        if (abs(Ki - OldKi) > AdjustLimit * 30) {
          PIDTuningTimer = 0;
          OldKi = Ki;
          EEPROM.put(510, Ki);
          if (DisplayID != 9) {
        ...
        // update the PID driver
        CyborgPID.SetTunings(Kp, Ki, Kd);

        // now we can display the changes
        if ((DisplayID == 9) && (DriverChangeScreen > 3000)) {
          if (RedrawDisplay) {
      ...
      // here's the deal... throttle output range is 1-4 VDC, MCU can accept up to 3v3 and output 3v3
      ...
      CyborgOutSignal = (CyborgOutputPWM * 100.0) / BIT_CONVERSION;
      CyborgOutSignal = FloatMap(CyborgOutSignal, 18.8f, 100.0f, 0.0f, 100.0f);
  ...
  if (SpeedUpdateTimer >= SpeedTimer) {
    ...
    RPMSum = 0;
    RPMCount = 0;
    if ((DriverChangeScreen > 3000) && (DisplayID == 3)) {
      if (RedrawDisplay) {
        Display.fillScreen(back_color);
        RedrawHeader = true;
        RedrawDisplay = false;
  ...
  if (DisplayUpdateTimer >= UPDATE_LIMIT) {
    ...
    if (RaceStatus == RACE_INPROGRESS) {
      ...
      if (KeyState == HIGH) {
        ...
        if (AveragingTypeID == 0) {
          ComputePredictionReg();
        } else {
          ComputePredictionPoint();
        }
        ...
        if (CurrentBias > 0.015f) {
          ...
          if (AveragingUpdateTimer > (AveragingUpdateTime * 1000)) {
            ...
            if ((PredictedEnergy - TotalEnergy) > ((LimitTolerance / 100.0f) * TotalEnergy)) {
              ...
              // need to increas CyborgFirstLimit
              CyborgFirstLimit = CyborgFirstLimit + CurrentBias;
            }
          }
        }
    ...
    if (DriverChangeScreen > 3000) {
      ...
      switch (DisplayID) {
        ...
        case 2:
          ...
          break;
        case 3:
          // this is now in loop so we can contol faster screen updates
          //SpeedView();
          break;
        case 4:
          TempView();
        ...
        case 6:
          ...
          break;
        case 7:
          // Show GFORCE (code in loop)
          break;
        case 8:
          // Show CYBORG screen and limit setting screen (code in loop)
          break;
        case 9:
          // Show CYBORG pid tuning scre (code in loop)
          break;
      }
      if (RaceStatus == RACE_INPROGRESS) {
...
void ComputePredictionPoint() {
  ...
  if (PredictionArrayCounter < PREDICT_SAMPLES) {
    ...
    m_PE = (EnergyArray[PREDICT_SAMPLES - 1] - EnergyArray[0]) / (PointArray[PREDICT_SAMPLES - 1] - PointArray[0]);
    b_PE = Energy - (m_PE * Point);
    ...
    for (iCP = 0; iCP < PREDICT_SAMPLES - 1; iCP++) {
      EnergyArray[iCP] = EnergyArray[iCP + 1];
      PointArray[iCP] = PointArray[iCP + 1];
    }
  ...
  Serial.print(Energy);
  Serial.print(", Time ");
  Serial.print(Point / 120.0f);
  Serial.print(", Energy Start ");
  Serial.print(EnergyArray[PREDICT_SAMPLES - 1]);
  Serial.print(", Energy End ");
  Serial.print(EnergyArray[0]);
  Serial.print(", Point Start ");
  Serial.print(PointArray[PREDICT_SAMPLES - 1]);
  Serial.print(", Point End ");
  Serial.print(PointArray[0]);
  Serial.print(", b_PE ");
  Serial.print(b_PE, 4);
  Serial.print(", m_PE ");
  Serial.print(m_PE, 4);
...
/*
   ...
   PARAMS :  -
   RETURNS : None
   NOTES : called in racehasstarted
*/
...
void GetParameters() {
  ...
  // could be due to corrupted eeprom data or added parameter and eeprom has some old data
    ...
    if (RestoreEEPROM) {
      ...
      AveragingTypeID = 0;
      EEPROM.put(540, AveragingTypeID);
  ...
  EEPROM.get(540, AveragingTypeID);
  ...
  Serial.print(F("EnablePIDTuning: "));
  Serial.println(EnablePIDTuning);
...
void CYBORGPIDTuneView() {
  ...
  if (GettingKd) {
    ...
    Display.print(F("Kd"));
    Display.setFont(FONT_24BI);
    ffCYBORGKp.setTextColor(C_DKGREY, back_color);
    ffCYBORGKi.setTextColor(C_DKGREY, back_color);
    ffCYBORGKd.setTextColor(fore_color, back_color);
    ...
    Display.print(F("Kd"));
    Display.setFont(FONT_24BI);
    ffCYBORGKp.setTextColor(fore_color, back_color);
    ffCYBORGKi.setTextColor(fore_color, back_color);
    Display.setCursor(40, 50);
    ffCYBORGKp.print(Kp, 1);
    Display.setCursor(260, 50);
    ffCYBORGKi.print(Ki, 0);
    ffCYBORGKd.setTextColor(C_DKGREY, back_color);
    Display.setCursor(140, 50);
    ffCYBORGKd.print(" ");
  }
...
void TimeView() {
  ...
  if (RaceStatus == RACE_INPROGRESS) {
    if (TimeDelta > 0) {
      ffTimeDelta.setTextColor(C_RED, back_color);
      sprintf(buf, "+%d", TimeDelta);
    } else if (TimeDelta < 0) {
      sprintf(buf, "-%d", abs(TimeDelta));
      ffTimeDelta.setTextColor(C_GREEN, back_color);
    } else {
      sprintf(buf, "%d", TimeDelta);
      ffTimeDelta.setTextColor(fore_color, back_color);
    }
...
void CreateUserInterface() {
  ...
  RaceMenuOption11 = RaceMenu.addNI("Tire pressure [psi]", TirePressure, 70, 200, 5);
  ...
  RaceMenuOption13 = RaceMenu.addNI("Lap Amp warning [a]", MaxLapAmps, 15, 25, .1, 1);
  ...
  RaceMenuOption15 = RaceMenu.addNI("GPS lap threshold [s]", LapThreshold, 10, 120, 5);
  ...
  SettingsMenuOption6 = SettingsMenu.addNI("Start/Driver trigger [a]", TriggerAmps, 10, 90, 5, 0);
  SettingsMenuOption1 = SettingsMenu.addNI("Motor", MotorID, 0, sizeof(MotorText) / sizeof(MotorText[0]), 1, 0, MotorText);
  SettingsMenuOption3 = SettingsMenu.addNI("Screen button location", Orientation, 0, sizeof(OrientationText) / sizeof(OrientationText[0]), 1, 0, OrientationText);
  SettingsMenuOption4 = SettingsMenu.addNI("Theme", Theme, 0, sizeof(ThemeText) / sizeof(ThemeText[0]), 1, 0, ThemeText);
  SettingsMenuOption5 = SettingsMenu.addNI("Car", CarID, 0, sizeof(CarText) / sizeof(CarText[0]), 1, 0, CarText);
  SettingsMenuOption9 = SettingsMenu.addNI("Restart display each draw", RestartDisplayAlways, 0, sizeof(YesNoText) / sizeof(YesNoText[0]), 1, 0, YesNoText);
  ...
  ClockMenu.init(MENU_TEXT, MENU_BACK, MENU_HIGHTEXT, MENU_HIGHLIGHT, MENU_SELECTTEXT, MENU_SELECT, 230, 22, 5, "Clock Setup", FONT_14, FONT_16B);
  ...
  CyborgMenuOption14 = CyborgMenu.addNI("Averaging Scheme", AveragingTypeID, 0, sizeof(AveragingTypeText) / sizeof(AveragingTypeText[0]), 1, 0, AveragingTypeText);
  CyborgMenuOption11 = CyborgMenu.addNI("Prediction Avg [s]", AveragingUpdateTime, 10, 180, 1, 0);
  CyborgMenuOption12 = CyborgMenu.addNI("Prediction Comp [A]", PredictionCompensation, 10, 180, 1, 0);
  CyborgMenuOption13 = CyborgMenu.addNI("Limit bias tolerance [%]", LimitTolerance, 0, 10, 0.1, 1);
  CyborgMenuOption1 = CyborgMenu.addNI("Race current [A]", CyborgFirstLimit, 8.0, 50.0, 0.1, 1);
  CyborgMenuOption9 = CyborgMenu.addNI("Turbo current [A]", CyborgSecondLimit, 8.0, 50.0, 0.1, 1);
  CyborgMenuOption10 = CyborgMenu.addNI("Enable PID tuning", EnablePIDTuning, 0, sizeof(YesNoText) / sizeof(YesNoText[0]), 1, 0, YesNoText);
  ...
  CyborgVoltsID = CYBORGPIDTuneG.add("PWM", C_RED);
...
void ProcessRaceMenu() {
  ...
  LapThreshold = (uint8_t)RaceMenu.value[RaceMenuOption15];  // LapThreshold, seconds GPS considers a lap
...
void ProcessSettingsMenu() {
  ...
  Orientation = (uint8_t)SettingsMenu.value[SettingsMenuOption3];
  Theme = (uint8_t)SettingsMenu.value[SettingsMenuOption4];
  CarID = (uint8_t)SettingsMenu.value[SettingsMenuOption5];
  TriggerAmps = (uint8_t)SettingsMenu.value[SettingsMenuOption6];
  RestartDisplayAlways = (bool)SettingsMenu.value[SettingsMenuOption9];
...
void ProcessCyborgMenu() {
  ...
  CyborgMenu.SetItemValue(CyborgMenuOption9, CyborgSecondLimit);
  ...
  while (MenuOption > 0) {
    ...
    if (ButtonPressed == C_BUTTON) {
      ...
      WaitForRelease();
      if (CyborgMenu.value[CyborgMenuOption2] == 0) {
        CyborgMenu.SetItemValue(CyborgMenuOption10, 0);
        CyborgMenu.drawRow(CyborgMenuOption10);
      }
      ExitStartTime = millis();
    }
  }
  ...
  EnablePIDTuning = CyborgMenu.value[CyborgMenuOption10];
  CyborgThresholdVolts = CyborgMenu.value[CyborgMenuOption7];
  AveragingUpdateTime = (uint8_t)CyborgMenu.value[CyborgMenuOption11];
  PredictionCompensation = CyborgMenu.value[CyborgMenuOption12];
  LimitTolerance = CyborgMenu.value[CyborgMenuOption13];
  ...
  AveragingTypeID = (uint8_t)CyborgMenu.value[CyborgMenuOption14];
...
void ProcessPlaybackMenu() {
  ...
  if (MaxRecordSets == 0) {
    ...
    Display.print("There are no");
    Display.setCursor(40, 100);
    Display.print("races to plot.");
...
void PlotRaceDataGraphing(uint8_t recID) {
  ...
  if (RPBPlotVolts) {
    ...
    Display.setTextColor(C_CYAN, back_color);
    Display.print(F("Volts"));
    ColStart = ColStart + 53;
  }
  if (RPBPlotAmps) {
    Display.setCursor(ColStart, 10);
    Display.setTextColor(C_YELLOW, back_color);
    Display.print(F("Amps"));
    ColStart = ColStart + 55;
  }
  if (RPBPlotLapAmps) {
    Display.setCursor(ColStart, 10);
    Display.setTextColor(C_LTPURPLE, back_color);
    Display.print(F("LAmps"));
    ColStart = ColStart + 65;
  }
  if (RPBPlotSpeed) {
    Display.setCursor(ColStart, 10);
    Display.setTextColor(C_GREEN, back_color);
    Display.print(F("Speed"));
    ColStart = ColStart + 65;
  }
  if (RPBPlotMTemp) {
  ...
  if (RPBPlotAltitude) {
    Display.setCursor(ColStart, 10);
    Display.setTextColor(C_PINK, back_color);
  ...
  if (RPBCyborgOut) {
    Display.setCursor(ColStart, 10);
    Display.setTextColor(C_LTBLUE, back_color);
...
void DownloadRaceData(uint32_t Count) {
  ...
  for (i = 1; i <= LastRecord; i++) {
    ...
    if (rt == RT_HEADER) {
      ...
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(F("Kp"));
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(F("Ki"));
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(F("Kd"));
      SDDataFile.write(DATA_DELIMITER);
    ...
    if (rt == RT_DATA) {
      ...
      if (EnableCyborg) {
        ...
        SDDataFile.write(DATA_DELIMITER);
        SDDataFile.print(SSD.getField(Kp, frKp));
        SDDataFile.write(DATA_DELIMITER);
        SDDataFile.print(SSD.getField(Ki, frKi));
        SDDataFile.write(DATA_DELIMITER);
        SDDataFile.print(SSD.getField(Kd, frKd));
        SDDataFile.write(DATA_DELIMITER);
...
void DownloadEEPROM() {
  ...
  SDDataFile.println(AveragingTypeText[(uint8_t)AveragingTypeID]);
...
void SetScreenParameters() {
  ...
  CYBORGPIDTuneG.setColors(fore_color, C_DKGREY, C_BLUE, back_color, back_color);
...
void BuildFieldList() {
  ...
  frKp = SSD.addField(&Kp);                              // RestoreType" 1
  frKi = SSD.addField(&Ki);                              // Cyborg Input" 1
  frKd = SSD.addField(&Kd);                              // Cyborg Output" 1
