...
#define CODE_VERSION "10.25.0021"
...
int8_t EnergyID, bEnergyID, GForceXID, GForceYID, GForceZID, GForceID, CyborgAmpsID;
int8_t GraphVoltsID, GraphAmpsID, GraphLapAmpsID, GraphSpeedID, GraphMTempID, GraphCyborgInID, GraphCyborgOutID;
uint8_t Button = 0;
...
float iTemp = 0.0f, StartPressure = 0.0f, Pressure = 0.0f, Humidity = 0.0f;
//Buttons
...
float AverageVolts = 0.0f, LapEnergy = 0.0f, StartLapEnergy = 0.0f, AverageSpeed = 0.0f;
...
void DownloadRaceData(uint32_t Count) {
  ...
  for (i = 1; i <= LastRecord; i++) {
    ...
    if (rt == RT_HEADER) {
      ...
      SDDataFile.write(DATA_DELIMITER);
      SDDataFile.print(F("GPS Speed [PMH]"));
      SDDataFile.write(DATA_DELIMITER);
